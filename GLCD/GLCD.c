...
void drawMatrix(){
	...
	for(i=30;i<=210;i+=12){ 								//disegna righe
			LCD_DrawLine(i,64,i,220,Black);
	}
	for(j=64;j<=220;j+=12){									//disegna colonne
			LCD_DrawLine(30,j,210,j,Black);
		}
}

void drawRobot(uint16_t color){
	int x = pos.y;
	int y = pos.x;
	...
	if(direction == 0){ //EAST
		...
		for(i=(30+12*x)+1;i<30+(12*(x+1));i++){
			LCD_DrawLine(i,start,i,end,color);
			if(flag==1){
				start++;
				end--;
				flag=0;
	...
	if(direction==1){  //SUD
		...
		for(i=64+12*y+1;i<64+12*(y+1);i++){
			
			LCD_DrawLine(start,i,end,i,color);
			if(flag==1){
				start++;
				end--;
				flag=0;
...
void clearPos(int x, int y){ //PULISCE UN QUADRATINO
	int i,j;
	for(i=30+x*12+1;i<30+(x+1)*12;i++){
		for(j=64+12*y+1;j<64+(y+1)*12;j++){
			LCD_SetPoint(i,j,White);
		}
	}
...
void drawObstacle(int x, int y){ //disegna un ostacolo
	int i,j;
	for(i=30+x*12+1;i<30+(x+1)*12;i++){
		for(j=64+12*y+1;j<64+(y+1)*12;j++){
...
void newGame(){ //inizia un nuovo game
	matrix_init();
	init_position();
	mode=1;
	disable_joystick=0;
...
void init_restart_clear(){
	int i;
	
	for(i=30;i<=110;i++)
		LCD_DrawLine(i,240,i,300,White);
	
	GUI_Text(40, 260, (uint8_t *) "Restart", Blue, White);
	
	for(i=130;i<=210;i++)
		LCD_DrawLine(i,240,i,300,White);
	
	GUI_Text(150, 260, (uint8_t *) "Clear", Blue, White);
}
